...
class Offersde extends React.Component{
  constructor(props){
    super(props);
    this.state={
      show:false,
  ...
  componentDidMount(){
        ...
        axios.get('http://3.14.168.174/campaign/getAllCampaignOfBranch/deal/'+this.props.match.params.id,{headers})
        .then((resp)=>{
  ...
  createfreshcamp=(e)=>{
    ...
    headers: {'content-type': 'application/json', Authorization: `${token}`},
     ...
     console.log(sendingdata)
    axios
    .post('http://3.14.168.174/campaign/add/offer_deal',sendingdata,config)
    .then(response => {
      console.log(response)
      ...
      if (response.data.code === 200) {
        ...
        .then(()=>{
          window.location.href = "/offersde/"+this.state.branchid,this.state={
...
<br/><br/>
  ...
  <div class="col" style={{
    ...
    <div class="col">
      <input type="number" style={{fontSize: "25px"}} class="form-control" placeholder="" onChange={this.handlebuy}/>
    </div><div class="col" style={{
          fontSize: "25px"
        }}><b>Get</b></div>
    <div class="col">
  ...
  <br/><br/>
  <button onClick={this.createfreshcamp}style={{ backgroundColor: "#ff4c44", borderRadius: "25px", height: "45px", fontSize: "25px", color: "white"}}> <Link   style={{textDecoration: "none", color: "white"}}>Create</Link></button>
</form>
        </div>
            ...
            <div className="row">
              <div className="col" style={{fontSize: '20px', boxShadow: '3px 2px 10px rgba(175, 175, 175, 0.685)', marginRight: '10px', height: '120px', background: 'white', borderRadius: '5px', alignItems: 'center'}}> <b className="com" style={{marginTop: "45px", marginLeft: "160px"}}><Link to={{pathname: "/offersdi/"+this.state.branchid+"/"+this.state.is, state: {
               usersdata: this.state.seleteditems
              }}} style={{textDecoration: "none", color: "red" }} >Offers Discount</Link></b></div>
              <div className="col" style={{fontSize: '20px', boxShadow: '3px 2px 10px rgba(175, 175, 175, 0.685)', marginRight: '10px', height: '120px', background: 'linear-gradient(0deg, rgba(255,51,0,0.9416141456582633) 0%, rgba(255,77,77,1) 100%)', borderRadius: '5px', alignItems: 'center'}}> <b className="com" style={{marginTop: "45px", marginLeft: "160px"}}><Link to={{
      pathname: `/offersde/`+this.state.branchid+"/"+this.state.is,
    state: {
      usersdata: this.state.seleteditems
    }
    }} style={{textDecoration: "none", color: "white" }}>Offers Deals</Link></b></div>
            </div>
  ...
  <div class= "container">
    ...
    <br/>
    <div class="container">
    <button style={{textAlign: "center", marginLeft: "400px", backgroundColor: "#ff4c44", borderRadius: "25px", height: "45px", fontSize: "25px", color: "white"}}> <Link  style={{textDecoration: "none", color: "white"}} onClick={this.startcamp}>Start Campaign </Link></button>
    </div>
            </div>
